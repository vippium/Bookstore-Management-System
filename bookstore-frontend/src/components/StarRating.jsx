...
export default function StarRating({ bookId, onRated, readOnly = false }) {
  ...
  const fetchRatings = async () => {
    try {
      ...
      const ratings = res.data?.ratings || []; // Access 'ratings' array
  ...
  const fetchUserRating = async () => {
    // Only fetch user rating if not in readOnly mode
    if (readOnly) return;
    ...
    try {
      ...
      if (found) {
        ...
        setUserReviewExists(true); // User has reviewed
      } else {
        setUserRating(0);
        setReview("");
        setUserReviewExists(false); // User has not reviewed
      }
    } catch (error) {
      // 401 Unauthorized errors are common if not logged in, suppress if expected
      if (error.response && error.response.status === 401) {
        // console.log("Not logged in, cannot fetch user rating for interactive component.");
      } else {
  ...
  return (
    <div className={`${readOnly ? "flex items-center gap-1" : "p-4 bg-gray-50 rounded-lg shadow-inner border border-gray-200"}`}>
      {!readOnly && (
        <h4 className="text-md font-semibold text-blue-600 mb-3">Your Rating & Review</h4>
      )}
      ...
      <div className={`flex gap-1 items-center ${!readOnly ? "mb-3" : ""}`}>
        {[1, 2, 3, 4, 5].map((val) => (
          <Star
            ...
            className={`${
              readOnly
                ? (avgRating && parseFloat(avgRating) >= val ? "text-yellow-500 fill-yellow-500" : "text-gray-300 fill-transparent")
                : ((hovered || userRating) >= val ? "text-yellow-500 fill-yellow-500" : "text-gray-300")
          ...
          <>
            {avgRating && parseFloat(avgRating) > 0 ? (
              <span className="text-sm font-semibold text-gray-700 ml-1">
                {avgRating} <span className="text-gray-500 text-xs">({count} ratings)</span>
              </span>
            ) : (
      ...
      {!readOnly && avgRating && (
        <div className="text-sm text-gray-600 mt-4 pt-3 border-t border-gray-200">
          Overall Average: <strong className="text-blue-700">{avgRating} out of 5</strong> from {count} {count === 1 ? "rating" : "ratings"}
        </div>
      )}
      {!readOnly && !userRating && (
        <p className="text-sm text-gray-500 mt-2">Select stars above to leave your rating and review!</p>
      )}
    </div>
  );
}