...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (success) {
        toast.success("Registration successful! Please login.", { icon: <CheckCircle size={20} className="text-green-500" /> });
        navigate("/login");
      } else {
        toast.error("Registration failed. Please try again.", { icon: <XCircle size={20} className="text-red-500" /> });
      }
    } catch (error) {
      console.error("Registration error:", error);
      toast.error(error.response?.data?.message || "An unexpected error occurred during registration.", { icon: <XCircle size={20} className="text-red-500" /> });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 px-4 py-8">
      <div className="w-full max-w-md p-8 bg-white rounded-2xl shadow-xl border border-blue-100 animate-fade-in-up">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-blue-700 text-center">Join Our Community</h2>
